...
def parse_taiwanese_date(date_str):
    """Converts 'YYY年MM月' to standard 'YYYY/MM/DD' datetime format (defaults to 1st of the month)."""
    if pd.isna(date_str):
        return pd.NaT
    ...
    try:
        if '年' in date_str and '月' in date_str:
            ...
            # Defaulting to the 1st day of the month to satisfy YYYY/MM/DD format
...
def process_data(files):
    ...
    for file in files:
        # Failsafe for encoding: Taiwanese CSVs are often encoded in Big5 or utf-8-sig
        try:
    ...
    # 2. Reorder columns (which implicitly swaps 重量(公斤) and 新臺幣(千元))
    final_order = [
        '進出口別', 
        '日期', 
        '貨品號列', 
    ...
    # Ensure only columns that actually exist in the CSV are filtered, protecting against KeyError
    ...
    if '日期' in combined_df.columns:
        # Create a hidden datetime column for accurate chronological sorting
        combined_df['_sort_date'] = combined_df['日期'].apply(parse_taiwanese_date)
        # Format the actual output column to the requested YYYY/MM/DD string
        combined_df['日期'] = combined_df['_sort_date'].dt.strftime('%Y/%m/%d')
    else:
        # Fallback if '日期' column is somehow missing
        combined_df['_sort_date'] = pd.NaT

    # 5. Force '貨品號列' to be recognized as a Number 
    if '貨品號列' in combined_df.columns:
        # Convert to numeric, and use Int64 to prevent decimals (.0) on product codes while supporting NaNs
        combined_df['貨品號列'] = pd.to_numeric(combined_df['貨品號列'], errors='coerce').astype('Int64')

    # 6. Sort Hierarchy
    # Create a logical category for sorting IMPORT above EXPORT
    if '進出口別' in combined_df.columns:
    ...
    # Sort by Earliest Date first, then IMPORT -> EXPORT
    combined_df = combined_df.sort_values(by=['_sort_date', '進出口別'])

    # Clean up hidden sorting column before exporting
